...
export async function GET(request: NextRequest) {
  try {
    ...
    // If no table data yet, try to migrate from blob
    if (!a.data?.length) {
      const blob = (s.data as any)?.data_blob
      if (blob?.artists?.length) {
        await saveAll(uid, blob)
        // Re-fetch artists after migration
        const a2 = await admin.from('artists').select('*').eq('user_id', uid).order('name')
        const t2 = await admin.from('tours').select('*').eq('user_id', uid).order('start_date')
...
function buildResponse(artists: any[], tours: any[], meetings: any[], replacements: any[], expenses: any[], contacts: any[], settings: any) {
  // Convert DB format back to HTML app format
  return NextResponse.json({
    ...
    calY: settings?.cal_year, calM: settings?.cal_month,
    // Keep these from blob for now
...
async function saveAll(uid: string, data: any): Promise<void> {
  const run = async (table: string, record: Record<string, unknown>) => {
    try { await admin.from(table).upsert(record) } catch(e) {}
  }

  for (const a of (data.artists || [])) {
    await run('artists', { id: String(a.id), user_id: uid, name: a.name, genre: a.genre||'', color: a.color||'#C9A84C', siret: a.siret||'', address: a.address||'', nature: a.nature||'' })
  }
  for (const t of (data.tours || [])) {
    await run('tours', { id: String(t.id), user_id: uid, artist_id: t.aId ? String(t.aId) : null, title: t.title||'Untitled', start_date: t.start, end_date: t.end||t.start, city: t.city||'', type: t.type||'show', paid: t.paid!==false, received: t.received||false, custom_cachet: t.customCachet||null, custom_hours: t.customHours||null, notes: t.notes||'', address: t.address||'', hotel: t.hotel||'', room: t.room||'', hotel_addr: t.hotelAddr||'', doclink: t.doclink||'' })
  }
  for (const m of (data.meetings || [])) {
    await run('meetings', { id: String(m.id), user_id: uid, title: m.title||'Meeting', type: m.type||'online', date: m.date, time: m.time||'', location: m.location||'', notes: m.notes||'' })
  }
  for (const s of (data.subs || [])) {
    await run('replacements', { id: String(s.id), user_id: uid, name: s.name, instrument: s.inst||'', phone: s.phone||'', genre: s.genre||'', notes: s.notes||'' })
  }
  for (const e of (data.expenses || [])) {
    await run('expenses', { id: String(e.id), user_id: uid, date: e.date, amount: e.amount, category: e.cat||'other', description: e.desc||'', receipt_url: e.receipt||'' })
  }
  for (const ct of (data.contacts || [])) {
    await run('contacts', { id: String(ct.id), user_id: uid, name: ct.name, role: ct.role||'', company: ct.company||'', contact_info: ct.contact||'', last_contact_date: ct.last||null, followup_date: ct.followup||null, notes: ct.notes||'' })
  }
  await run('user_settings', { user_id: uid, hours_goal: data.hoursGoal||507, hour_types: data.hoursPerEventType||{}, cal_year: data.calY, cal_month: data.calM, updated_at: new Date().toISOString() })
}
